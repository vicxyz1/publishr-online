...
<div class="section">
    <div class="container">
        ...
        <?php if (isset($this->err1_msg)): ?>

            <div class="alert alert-dismissable alert-danger"><?= $this->err1_msg ?></div>
        ...
        <?php if (count($this->unpublished)): ?>
            ...
            <div class="form-group">
                <select multiple="multiple" class="image-picker show-html " name="photos[]" required>
                    <?php foreach ($this->unpublished as $photo): ?>
                        <option data-img-src="<?= $photo['url_q'] ?>" value="<?= $photo['id'] ?>" data-img-label='<a data-toggle="lightbox" href="<?= $photo['url_z'] ?>"><?= $photo['title'] ?></a>'><a href="<?= $photo['url_o'] ?>"><?= $photo['title'] ?></a></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="container">
        ...
        <div class="container">
            <div class="row">
                <div class='col-md-6'>
                    <div class="form-group">
                        <label for="datetimepicker1" class="control-label"> Select when you make the photos public</label>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <select id="groups" multiple="multiple" class="image-picker show-html " name="groups[]" required>
                            <?php if (count($this->groups)) foreach ($this->groups as $id => $group): ?>
                            <?php $src= !$group['iconfarm']?'https://s.yimg.com/pw/images/buddyicon03_r.png':"http://farm{$group['iconfarm']}.staticflickr.com/{$group['iconserver']}/buddyicons/{$group['nsid']}.jpg"?>
                                    <option width=48 data-img-src="<?=$src?>" value="<?= $group['id'] ?>"><?= $group['name'] ?></option>
                                <?php endforeach ?>
                        </select>


                    </div>
                    <div class="modal-footer"><a id="btnGroups" class="btn btn-primary" data-dismiss="modal">Save changes</a></div>
                </div>
            </div>
        </div>
        <div class="section">
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-md-12r text-center">
                        <a class="btn btn-default btn-lg" data-toggle="modal" data-target="#setGroups"><i class="fa fa-star fa-fw"></i>Add to Groups</a>
    ...
    <div class="section">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <a name="scheduled"></a><h1 class="text-left">Scheduled Photos</h1>
                    <p class="lead text-left">select photos to cancel the publishing</p>
                </div>
            </div>
            <?php if (isset($this->err2_msg)): ?>

                <div class="alert alert-dismissable alert-danger"><?= $this->err2_msg ?></div>

            <?php endif ?>

            <?php if (count($this->scheduled)) foreach ($this->scheduled as $date => $photos_scheduled): ?>
                    ...
                    <select multiple="multiple" class="image-picker show-html" name="photos[]">
                        <?php foreach ($photos_scheduled as $photo): ?>
                            <option data-img-src="<?= $photo['url_q'] ?>" value="<?= $photo['id'] ?>"><?= $photo['title'] ?></option>
                        <?php endforeach; ?>
                    </select>
                    </form>
...
<script type="text/javascript">
    ...
    var uoptions = {
        totalPages: <?= $this->upages ?>,
        visiblePages: 8,
        href: '?upage={{number}}'
    }

    var soptions = {
        totalPages: <?= $this->spages ?>,
        visiblePages: 8,
